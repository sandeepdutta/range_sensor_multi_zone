...
static uint8_t vl53l5cx_program_output_config(
		VL53L5CX_Configuration 		 *p_dev)
{
	...
	for (i = 0; i < (uint32_t)(sizeof(output)/sizeof(uint32_t)); i++)
	{
                    ...
                    && ((uint8_t)bh_ptr->header_bits.type < (uint8_t)0x0d))
		{
			...
			{
				bh_ptr->header_bits.size = (uint8_t)(resolution
		...
		uint16_t			distance_mm)
{
	...
	else
	{
		...
		status |= vl53l5cx_poll_for_answer(p_dev,
		...
		status |= vl53l5cx_program_output_config(p_dev);
	...
	status |= vl53l5cx_poll_for_answer(p_dev,VL53L5CX_UI_CMD_STATUS, 0x03);
