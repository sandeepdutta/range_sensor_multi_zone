...
#include <range_sensor_multi_zone/vl53l5cx_buffers.h>
#include <stdio.h>
/**
...
static uint8_t vl53l5cx_poll_for_answer(
 ...
 */
static uint8_t vl53l5cx_poll_for_mcu_boot(
              VL53L5CX_Configuration      *p_dev)
{
...
static uint8_t vl53l5cx_send_offset_data(
		VL53L5CX_Configuration		*p_dev,
		uint8_t						resolution)
{
	uint8_t status = VL53L5CX_STATUS_OK;
	...
	status |=vl53l5cx_poll_for_answer(p_dev, 4, 1,
...
static uint8_t vl53l5cx_send_xtalk_data(
		VL53L5CX_Configuration		*p_dev,
		uint8_t				resolution)
{
	uint8_t status = VL53L5CX_STATUS_OK;
		...
		VL53L5CX_Configuration		*p_dev)
{
	...
	status |= vl53l5cx_poll_for_answer(p_dev, 4, 0,
		...
		uint8_t			        power_mode)
{
	...
	switch(power_mode)
	{
		case VL53L5CX_POWER_MODE_WAKEUP:
			...
			status |= vl53l5cx_poll_for_answer(
		...
		VL53L5CX_Configuration		*p_dev)
{
	...
	for (i = 0; i < (uint32_t)(sizeof(output)/sizeof(uint32_t)); i++)
	{
		...
		if (((uint8_t)bh_ptr->header_bits.type >= (uint8_t)0x1)
                    && ((uint8_t)bh_ptr->header_bits.type < (uint8_t)0x0d))
		{
			if ((bh_ptr->header_bits.idx >= (uint16_t)0x54d0)
                            && (bh_ptr->header_bits.idx < (uint16_t)(0x54d0 + 960)))
			{
				bh_ptr->header_bits.size = resolution;
			}
			else
			{
				bh_ptr->header_bits.size = (uint16_t)((uint16_t)resolution
                                  * (uint16_t)VL53L5CX_NB_TARGET_PER_ZONE);
			}
			p_dev->data_read_size += bh_ptr->header_bits.type * bh_ptr->header_bits.size;
		}
		else
		{
			p_dev->data_read_size += bh_ptr->header_bits.size;
		}
		...
		VL53L5CX_ResultsData		*p_results)
{
             ...
             < (uint32_t)p_dev->data_read_size; i+=(uint32_t)4)
	{
		...
		if ((bh_ptr->header_bits.type > (uint32_t)0x1)
                    && (bh_ptr->header_bits.type < (uint32_t)0xd))
		{
			msize = bh_ptr->header_bits.type * bh_ptr->header_bits.size;
		}
		else
		{
			msize = bh_ptr->header_bits.size;
		}

		switch(bh_ptr->header_bits.idx){
